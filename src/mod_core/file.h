...
public:
    ...
    static int Type(const String& filename) { return platform_FileType(filename); }
    static unsigned long Size_(const String& filename) { return platform_FileSize(filename); }
    static void Copy(const String& filenameSrc, const String& filenameDst);
    static void Delete(const String& filename) { platform_DeleteFile(filename); }
private:
    FILE* handle;
    unsigned long size;
};
...
inline void File::Copy(const String& filenameSrc, const String& filenameDst) {
    // Open first file for reading
    File fin(filenameSrc, FILE_READ);
    if ( fin.IsOpened() ) {
        // Read contents
        ...
        File fout(filenameDst, FILE_WRITE);
        if ( fout.IsOpened() ) {
