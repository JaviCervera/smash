...
void ScriptBuilder::SetIncludeCallback(INCLUDECALLBACK_t callback, void *userParam)
{
	includeCallback = callback;
	callbackParam   = userParam;
}

int ScriptBuilder::StartNewModule(asIScriptEngine *engine, const char *moduleName)
{
...
int ScriptBuilder::AddSectionFromMemory(const char *sectionName, const char *scriptCode, unsigned int scriptLength)
{
...
int ScriptBuilder::ProcessScriptSection(const char *script, unsigned int length, const char *sectionname)
{
			...
			if( t == asTC_IDENTIFIER )
			{
                ...
                else if( token == "load" )
                {
                    ...
                    if( t == asTC_VALUE && len > 2 && (modifiedScript[pos] == '"' || modifiedScript[pos] == '\'') )
                    {
                        // Get the module name
                        string modulename;
                        modulename.assign(&modifiedScript[pos+1], len-2);
                        pos += len;

                        // Store it for later processing
                        loadedModules.Add(modulename.c_str());
